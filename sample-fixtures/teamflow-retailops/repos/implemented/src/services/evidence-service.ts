import { config, evidenceDownloadLinks, evidenceFiles, tasks } from "../data/store.js";
import type { EvidenceDownloadLink, EvidenceFile, User } from "../domain/types.js";
...
export function createEvidenceDownloadLink(actor: User, evidenceId: string, now = new Date()): EvidenceDownloadLink {
  ...
  if (!task) throw new AppError("Task not found", 404);
  if (!canReadTask(actor, task)) throw new AppError("You cannot access this evidence", 403);

  const token = createId("download");
  const expiresAt = new Date(now.getTime() + config.downloadUrlTtlMinutes * 60 * 1000);
  const link: EvidenceDownloadLink = {
    evidenceId,
    token,
    url: `/evidence/${evidenceId}/download?token=${token}`,
    expiresAt,
    createdBy: actor.id,
  };

  evidenceDownloadLinks.push(link);
  return link;
}

export function validateEvidenceDownloadToken(evidenceId: string, token: string, now = new Date()): EvidenceFile {
  const link = evidenceDownloadLinks.find((item) => item.evidenceId === evidenceId && item.token === token);
  if (!link) throw new AppError("Download link is invalid", 403);
  if (link.expiresAt.getTime() < now.getTime()) throw new AppError("Download link has expired", 403);
