...
이 디렉터리는 CodeMatch AI를 단일 분석 에이전트 구조에서 3단계 멀티 에이전트 구조로 확장하기 위한 한글 설계 문서 모음이다. 현재 앱 코드는 단일 `AI analyzer`를 중심으로 동작하지만, 이 문서들은 이후 구현자가 에이전트 역할, 프롬프트, 출력 계약, 탐지 기준을 바로 코드에 반영할 수 있도록 기준을 고정한다.
